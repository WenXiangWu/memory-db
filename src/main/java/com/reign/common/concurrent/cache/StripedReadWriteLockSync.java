 ...
 */
package com.reign.common.concurrent.cache;

import com.reign.common.concurrent.ConcurrencyUtil;
 ...
 * Rather than locking a cache or a store,
...
public class StripedReadWriteLockSync {
    ...
    public StripedReadWriteLockSync(int numberOfStripes) {
        if ((numberOfStripes & (numberOfStripes - 1)) != 0) {
            throw new IllegalArgumentException("Cannot create a CacheLockProvider with a non power-of-two number of stripes");
        }
        if (numberOfStripes == 0) {
            throw new IllegalArgumentException("A zero size CacheLockProvider does not have useful semantics.");
        }
